...
Net_Hunter::Net_Hunter(int sectors) {
    for (int i = 0; i < sectors; ++i) {
        layer_0.emplace_back(i, true);
        layer_1_x.emplace_back('x');
        layer_1_y.emplace_back('y');
    }
}

void Net_Hunter::setSectorPoints(std::map<int, std::vector<std::pair<double, double>>>* ptr) {
    sector_points_ptr = ptr;
}

void Net_Hunter::setCriticLearning(bool enabled) {
...
std::vector<double> Net_Hunter::process(const std::map<std::string, double>& input_map) {
    ...
    double target_x = layer_1_x[best_sector_id].chooseBestCoordinate(input_augmented, sector_points.at(best_sector_id), best_sector_id);
    double target_y = layer_1_y[best_sector_id].chooseBestCoordinate(input_augmented, sector_points.at(best_sector_id), best_sector_id);

    double dist = EnvUtils::distance(target_x, target_y, input_map.at("runner_x"), input_map.at("runner_y"));
    double layer1_reward = std::clamp((800.0 - dist) / 800.0, 0.0, 1.0);
...
int Net_Hunter::getChosenSector(const std::map<std::string, double>& input_map) {
    double best_score = -1e9;
    int best_sector_id = 0;
    for (int i = 0; i < layer_0.size(); ++i) {
        double score = layer_0[i].calculate(input_map, i);
...
void Net_Hunter::saveToFile(const std::string& path) const {
...
void Net_Hunter::loadFromFile(const std::string& path) {
    std::ifstream in(path);
    size_t count = 0;
    in >> count;

    if (!in.good() || count == 0 || count > 1000) {
        std::cerr << "[Net_Hunter] Invalid layer count: " << count << "\n";
        return;
    }
...
void Net_Hunter::loadFromFileWithoutCritic(const std::string& path) {
    std::ifstream in(path);
